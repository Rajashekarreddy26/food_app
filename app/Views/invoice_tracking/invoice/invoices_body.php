...
$payment_status_val = isset($params['payment_status']) ? $params['payment_status'] : 0;4
?>
<div class="clearfix">
    <form id="inv-ser" onsubmit="invoiceBody(); return false">
        ...
        <div class="float-end d-flex align-items-center">
            <div class="me-1">
                <small><label class="form-label mb-0">&nbsp;</label></small>
                <div>
                    <button type="button" class="btn btn-sm btn-primary" onclick="exportInvoices('<? echo $sort_column; ?>','<? echo $sort_order; ?>')"><i class="bi bi-upload"></i>&nbsp;Export</button>
...
<div class="table table-responsive mt-2 freeze-table printable" style="min-height: 400px; font-size: 12px;">
...
<div class="card-group">
    ...
    <div class="card text-bg-warning">
        <div class="card-body">
            <? $deduction_amt = $totals['tds_sum'] + $totals['tds_sgst_sum'] + $totals['tds_cgst_sum'] + $totals['labour_cess_sum'];
                $statutory_amt = $total_invoice_value - $deduction_amt; ?>
            <h5 class="card-title text-end fs-5">Cr. <?= displayNumber(($statutory_amt / 10000000), 2) ?></h5>
            <p class="card-text">
                Basic + TAX - Statutory&nbsp;
                <a class="text-black" data-bs-container="body" data-bs-toggle="popover" data-bs-trigger="hover focus" data-bs-placement="top" data-html="true" data-bs-content="BASIC : <?= displayNumber($totals['basic_sum']/10000000, 2); ?>, SGST: <?= displayNumber(($totals['sgst_sum'] / 10000000), 2) ?>, CGST: <?= displayNumber(($totals['cgst_sum'] / 10000000), 2) ?>,TDS: <?= displayNumber(($totals['tds_sum'] / 10000000), 2) ?>, SGST-TDS: <?= displayNumber(($totals['tds_sgst_sum'] / 10000000), 2) ?>, CGS-TDS: <?= displayNumber(($totals['tds_cgst_sum'] / 10000000), 2) ?>, Labour Cess: <?= displayNumber(($totals['labour_cess_sum'] / 10000000), 2) ?>"><i class="bi bi-info-circle"></i></a>
            </p>
        </div>
    </div>
    <div class="card text-bg-primary">
        <div class="card-body">
            ...
            <p class="card-text">
                Receivable including GST&nbsp;
                <a class="text-black" data-bs-container="body" data-bs-toggle="popover" data-bs-trigger="hover focus" data-bs-placement="top" data-html="true" data-bs-content="BASIC: <?= displayNumber($totals['basic_sum']/10000000, 2); ?>, SGST: <?= displayNumber(($totals['sgst_sum'] / 10000000), 2) ?>, CGST: <?= displayNumber(($totals['cgst_sum'] / 10000000), 2) ?>"><i class="bi bi-info-circle"></i></a>
            </p>
        </div>
    </div>
    ...
    <div class="card text-bg-secondary">
        <div class="card-body">
            ...
            <p class="card-text">Hold / Deductions&nbsp;
                <a class="text-white" data-bs-container="body" data-bs-toggle="popover" data-bs-trigger="hover focus" data-bs-placement="top" data-html="true" data-bs-content="Other Deductions: <?= displayNumber(($totals['other_deductions_sum'] / 10000000), 2) ?>"><i class="bi bi-info-circle"></i></a>
            </p>
        </div>
    </div>
    <!-- <div class="card text-bg-primary">
    ...
    <div class="card text-bg-danger">
        <div class="card-body">
            <? $bal_amt = $statutory_amt - $totals['received_sum'] - $totals['other_deductions_sum']; ?>
            <h5 class="card-title text-end fs-5">Cr. <?= displayNumber(($bal_amt / 10000000), 2) ?></h5>
